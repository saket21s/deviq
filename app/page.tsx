...
/* ═══════════════════════════════════════════════════
   STAT ROW
═══════════════════════════════════════════════════ */
function StatRow({ label, value, accent, tk, last, stripe }) {
  return (
    <div style={{
      display:"flex", alignItems:"center", justifyContent:"space-between",
      padding:"10px 16px",
      background: stripe ? tk.statStripe : "transparent",
        ...
        <div key={lang} style={{ marginBottom:9 }}>
          ...
          <div style={{ height:4, borderRadius:99, background:tk.ringTrack, overflow:"hidden" }}>
            <div style={{
              ...
              width:`${(cnt/max)*100}%`,
              background:`linear-gradient(90deg, ${tk.blue}, ${tk.teal})`,
  ...
  return (
    <div
      ...
      style={{
        background: tk.card,
        borderRadius: 20,
        border: `1px solid ${hov ? accentBorder : tk.cardBorder}`,
        overflow:"hidden",
        boxShadow: hov ? `${tk.shadow2}, 0 0 0 1px ${accentBorder}` : tk.shadow1,
...
function InputField({ label, desc, color, colorLight, colorBorder, value, onChange, onEnter, tk }) {
  ...
  return (
    <div style={{
      background: tk.card,
      border:`1.5px solid ${focused ? colorBorder : tk.cardBorder}`,
      borderRadius:16,
      overflow:"hidden",
      boxShadow: focused ? `0 0 0 3px ${colorLight}, ${tk.shadow1}` : tk.shadow1,
      ...
      <div style={{ display:"flex", alignItems:"center", padding:"13px 16px", gap:6 }}>
        ...
        <input
          ...
          style={{
            ...
            fontSize:14, fontWeight:600, color:tk.ink,
          }}
        />
      </div>
    </div>
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:0, pointerEvents:"none", overflow:"hidden" }}>
      ...
      <div style={{
        ...
        borderRadius:"50%", top:"-30%", left:"50%", transform:"translateX(-50%)",
        background:`radial-gradient(circle, ${tk.meshA} 0%, transparent 70%)`,
      }}/>
      <div style={{
        position:"absolute", width:"50vw", height:"50vw", maxWidth:600, maxHeight:600,
        borderRadius:"50%", bottom:"5%", right:"-10%",
        background:`radial-gradient(circle, ${tk.meshB} 0%, transparent 70%)`,
      }}/>
      <div style={{
        position:"absolute", width:"40vw", height:"40vw", maxWidth:500, maxHeight:500,
        borderRadius:"50%", top:"40%", left:"-8%",
        background:`radial-gradient(circle, ${tk.meshC} 0%, transparent 70%)`,
      }}/>
      {/* Subtle grid */}
      <div style={{
        position:"absolute", inset:0,
        backgroundImage:`linear-gradient(${tk.divider} 1px, transparent 1px), linear-gradient(90deg, ${tk.divider} 1px, transparent 1px)`,
        backgroundSize:"60px 60px",
        opacity: tk.pageBg === "#09090B" ? 0.3 : 0.4,
...
export default function Page() {
  ...
  const analyze = async () => {
    ...
    const init = [
      ...(gh ? [{ label:"GitHub",     status:"pending" }] : []),
      ...(lc ? [{ label:"LeetCode",   status:"pending" }] : []),
      ...(cf ? [{ label:"Codeforces", status:"pending" }] : []),
    ];
    let cur = [...init]; setSteps(cur);
    const upd = (i, s) => { cur = cur.map((x,j) => j===i ? {...x,status:s} : x); setSteps([...cur]); };
    const errs = [], result = {}; let si = 0;
    if (gh) {
      upd(si,"active");
      ...
      catch(e) { upd(si,"error"); errs.push(`GitHub: ${e.message}`); } si++;
    }
    if (lc) {
      upd(si,"active");
      ...
      catch(e) { upd(si,"error"); errs.push(`LeetCode: ${e.message}`); } si++;
    }
    if (cf) {
      upd(si,"active");
      ...
      catch(e) { upd(si,"error"); errs.push(`Codeforces: ${e.message}`); }
    }
    let score = 0;
    ...
    setErrors(errs); setData(result); setLoading(false);
  };

  const sIcon = s => ({ done:"✓", error:"✗", active:"◉", pending:"○" }[s]);
  const stepColor = s => s==="active" ? tk.blue : s==="done" ? tk.green : s==="error" ? tk.rose : tk.ink4;
  const stepBg    = s => s==="active" ? tk.blueLight : s==="done" ? tk.greenLight : s==="error" ? tk.roseLight : "transparent";
  const stepBdr   = s => s==="active" ? tk.blueBorder : s==="done" ? tk.greenBorder : s==="error" ? tk.roseBorder : tk.cardBorder;
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        .scalePop3 { animation-delay:0.3s; }
        .hero-num {
        ...
        <div style={{ position:"relative", zIndex:1, maxWidth:1100, margin:"0 auto", padding:"0 28px 120px" }}>
          ...
          <header className="fadeUp" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"24px 0", marginBottom:72, borderBottom:`1px solid ${tk.divider}` }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:12 }}>
              <div style={{
                ...
                background: `linear-gradient(135deg, ${tk.blue}, ${tk.purple})`,
                display:"flex", alignItems:"center", justifyContent:"center",
                boxShadow:`0 4px 12px ${tk.blueGlow}`,
              }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:7, padding:"7px 14px", borderRadius:99, background:tk.pillBg, border:`1px solid ${tk.cardBorder}`, fontSize:11, fontWeight:600, color:tk.ink2 }}>
                <div style={{ width:7, height:7, borderRadius:"50%", background:tk.green, boxShadow:`0 0 8px ${tk.green}`, animation:"pulse 2s ease-in-out infinite" }}/>
                All systems live
              </div>
              ...
              <button
                ...
                style={{
                  ...
                  color:tk.ink2, letterSpacing:"0.02em",
                  transition:"all 0.2s", boxShadow:tk.shadow1,
                }}
              >
          ...
          <section style={{ marginBottom:80, display:"grid", gridTemplateColumns:"1fr auto", alignItems:"end", gap:40 }}>
            <div>
              <div className="fadeUp" style={{ display:"inline-flex", alignItems:"center", gap:8, marginBottom:20, padding:"6px 14px", borderRadius:99, background:tk.blueLight, border:`1px solid ${tk.blueBorder}` }}>
                <div style={{ width:5, height:5, borderRadius:"50%", background:tk.blue, animation:"pulse 2s ease-in-out infinite" }}/>
              ...
              <h1 className="fadeUp fadeUp1" style={{ fontFamily:"'Clash Display',sans-serif", fontWeight:700, fontSize:"clamp(44px,6vw,80px)", lineHeight:1.0, letterSpacing:"-0.04em", color:tk.ink, marginBottom:24 }}>
                ...
                <span style={{
                  display:"block",
                  backgroundImage:`linear-gradient(135deg, ${tk.blue} 0%, ${tk.purple} 50%, ${tk.teal} 100%)`,
                  WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text",
                }}>developer score.</span>
              </h1>
          ...
          <div className="fadeUp fadeUp2" style={{ marginBottom:56 }}>
            ...
            <button
              ...
              style={{
                ...
                background: (loading||(!gh&&!lc&&!cf)) ? tk.inset : `linear-gradient(135deg, ${tk.blue}, ${tk.purple})`,
            ...
            >
              {!loading && !(!gh&&!lc&&!cf) && (
                <div style={{ position:"absolute", inset:0, background:"linear-gradient(90deg, transparent, rgba(255,255,255,0.1), transparent)", animation:"shimmer 3s ease-in-out infinite" }}/>
              )}
              <span style={{ position:"relative" }}>
                {loading ? "Analyzing profiles…" : "⚡  Run Intelligence Analysis"}
              </span>
          ...
          {loading && (
            <div className="fadeUp" style={{
              background:tk.card, border:`1px solid ${tk.cardBorder}`, borderRadius:24,
              padding:"64px 40px", textAlign:"center", marginBottom:24, boxShadow:tk.shadow2,
            }}>
              ...
              <div style={{ display:"flex", justifyContent:"center", gap:8, flexWrap:"wrap" }}>
                {steps.map((s,i) => (
                  <div key={i} style={{
                    ...
                    padding:"7px 16px", borderRadius:99,
                    border:`1.5px solid ${stepBdr(s.status)}`,
                    color:stepColor(s.status), background:stepBg(s.status),
                    transition:"all 0.3s",
                  }}>
          ...
          {data && !loading && (() => {
            ...
            return (
              <>
                {/* ── SCORE BANNER ── */}
                <div className="scalePop" style={{
                  background: tk.card,
                  border:`1px solid ${tk.cardBorder}`,
                  borderRadius:24, marginBottom:16,
                  boxShadow:tk.shadow3,
                  overflow:"hidden", position:"relative",
                }}>
                  {/* Top gradient line */}
                  <div style={{ height:3, background:`linear-gradient(90deg, ${tk.blue}, ${tk.purple}, ${tk.teal})` }}/>
                  ...
                  <div style={{ padding:"36px 40px", display:"grid", gridTemplateColumns:"1fr auto", gap:32, alignItems:"center" }}>
                    <div>
                      ...
                      <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:28 }}>
                        {data.github     && <span style={{ fontSize:10, fontWeight:700, letterSpacing:"0.06em", textTransform:"uppercase", padding:"5px 13px", borderRadius:6, border:`1px solid ${tk.blueBorder}`,   color:tk.blue,   background:tk.blueLight   }}>GitHub ✓</span>}
                        {data.leetcode   && <span style={{ fontSize:10, fontWeight:700, letterSpacing:"0.06em", textTransform:"uppercase", padding:"5px 13px", borderRadius:6, border:`1px solid ${tk.amberBorder}`,  color:tk.amber,  background:tk.amberLight  }}>LeetCode ✓</span>}
                        {data.codeforces && <span style={{ fontSize:10, fontWeight:700, letterSpacing:"0.06em", textTransform:"uppercase", padding:"5px 13px", borderRadius:6, border:`1px solid ${tk.purpleBorder}`, color:tk.purple, background:tk.purpleLight }}>Codeforces ✓</span>}
                        <span style={{ fontSize:10, fontWeight:700, letterSpacing:"0.06em", textTransform:"uppercase", padding:"5px 13px", borderRadius:6, border:`1px solid ${rank.border}`, color:rank.color, background:rank.bg }}>
                  ...
                  <PlatformCard title="GitHub" handle={gh} accentColor={tk.blue} accentLight={tk.blueLight} accentBorder={tk.blueBorder} tk={tk} empty={!data.github?.analytics}>
                    {data.github?.analytics && (
                      <>
                        ...
                        ].map((s,i) => <StatRow key={s.label} label={s.label} value={s.value} accent={s.accent} tk={tk} last={false} stripe={i%2===1}/>)}
                    ...
                    {data.codeforces?.rating !== undefined && (
                      <>
                        {[
                          ...
                          { label:"Contribution",    value:data.codeforces.contribution,          accent:data.codeforces.contribution>=0 ? tk.green : tk.rose },
                        ].map((s,i) => <StatRow key={s.label} label={s.label} value={s.value} accent={s.accent} tk={tk} last={false} stripe={i%2===1}/>)}
                      </>
                    )}
                  </PlatformCard>
                ...
                {data.github?.repositories?.length > 0 && (
                  <div className="scalePop scalePop1" style={{ background:tk.card, border:`1px solid ${tk.cardBorder}`, borderRadius:20, overflow:"hidden", boxShadow:tk.shadow2, marginBottom:16 }}>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))" }}>
                      {data.github.repositories.map((r,i) => (
                        <RepoCard key={i} repo={r} tk={tk}/>
                      ))}
                    </div>
                  </div>
                )}
                ...
                <div style={{ padding:"18px 0", borderTop:`1px solid ${tk.divider}`, display:"flex", justifyContent:"space-between", alignItems:"center", fontSize:11, color:tk.ink3, fontWeight:600 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    <div style={{ width:20, height:20, borderRadius:5, background:`linear-gradient(135deg,${tk.blue},${tk.purple})`, display:"flex", alignItems:"center", justifyContent:"center" }}>
  ...
  return (
    <div
      ...
      style={{
        ...
        borderBottom:`1px solid ${tk.divider}`,
        background: hov ? tk.inset : "transparent",
