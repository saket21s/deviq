...
const THEMES = {
  light: {
    pageBg:"#FAFAF7", meshA:"rgba(99,102,241,0.06)", meshB:"rgba(16,185,129,0.05)", meshC:"rgba(245,158,11,0.04)",
    card:"#FFFFFF", cardHov:"#FDFDFB", cardBorder:"#E8E6E0", cardBorder2:"#D4D1C8", inset:"#F5F4F0",
    ink:"#18181B", ink2:"#52525B", ink3:"#A1A1AA", ink4:"#D4D4D8",
    blue:"#4F46E5", blueGlow:"rgba(79,70,229,0.15)", blueLight:"#EEF2FF", blueBorder:"#C7D2FE",
    ...
    ringTrack:"#E4E4E7", divider:"#E8E6E0", statStripe:"#FAFAFA",
  },
  dark: {
    pageBg:"#09090B", meshA:"rgba(99,102,241,0.12)", meshB:"rgba(16,185,129,0.08)", meshC:"rgba(245,158,11,0.06)",
    card:"#111114", cardHov:"#16161A", cardBorder:"#27272A", cardBorder2:"#3F3F46", inset:"#18181B",
    ink:"#FAFAFA", ink2:"#A1A1AA", ink3:"#52525B", ink4:"#3F3F46",
    blue:"#818CF8", blueGlow:"rgba(129,140,248,0.2)", blueLight:"rgba(129,140,248,0.08)", blueBorder:"rgba(129,140,248,0.25)",
    teal:"#2DD4BF", tealLight:"rgba(45,212,191,0.08)", tealBorder:"rgba(45,212,191,0.25)",
    amber:"#FCD34D", amberLight:"rgba(252,211,77,0.08)", amberBorder:"rgba(252,211,77,0.25)",
    rose:"#FB7185", roseLight:"rgba(251,113,133,0.08)", roseBorder:"rgba(251,113,133,0.25)",
    purple:"#C084FC", purpleLight:"rgba(192,132,252,0.08)", purpleBorder:"rgba(192,132,252,0.25)",
    green:"#34D399", greenLight:"rgba(52,211,153,0.08)", greenBorder:"rgba(52,211,153,0.25)",
    pillBg:"#18181B",
    ...
    ringTrack:"#27272A", divider:"#27272A", statStripe:"#0D0D10",
  },
};
...
/* ═══════════════════════════════════════════════════
   SCORE RING
═══════════════════════════════════════════════════ */
function ScoreRing({ score, tk, size = 148 }: { score: number; tk: Theme; size?: number }) {
  const c = useCounter(Math.round(score));
  const R = size * 0.365, circ = 2 * Math.PI * R;
  const pct = c / 100;
  ...
  return (
    <div style={{ position:"relative", width:size, height:size }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ transform:"rotate(-90deg)", display:"block" }}>
        ...
        <circle cx={cx} cy={cy} r={R} fill="none" stroke={tk.ringTrack} strokeWidth="6"/>
        <circle cx={cx} cy={cy} r={R} fill="none" stroke={color} strokeWidth="10"
          strokeLinecap="round" strokeDasharray={circ} strokeDashoffset={circ - pct * circ}
          opacity="0.15" style={{ filter:"blur(3px)", transition:"stroke-dashoffset 1.6s cubic-bezier(0.34,1.56,0.64,1)" }}
        />
        <circle cx={cx} cy={cy} r={R} fill="none" stroke="url(#ringGrad)" strokeWidth="6"
          strokeLinecap="round" strokeDasharray={circ} strokeDashoffset={circ - pct * circ}
          style={{ transition:"stroke-dashoffset 1.6s cubic-bezier(0.34,1.56,0.64,1)" }}
        />
      </svg>
      <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:2 }}>
        <span style={{ fontFamily:"'Clash Display',sans-serif", fontSize:size * 0.27, fontWeight:700, color, lineHeight:1, letterSpacing:"-0.03em" }}>{c}</span>
        <span style={{ fontSize:size * 0.075, color:tk.ink3, fontWeight:600, letterSpacing:"0.1em" }}>/ 100</span>
      </div>
    </div>
  );
}
...
function StatRow({ label, value, accent, tk, stripe }: {
  label: string; value: string|number|undefined; accent?: string|null; tk: Theme; stripe: boolean;
}) {
  return (
...
function PlatformCard({ title, handle, accentColor, accentLight, accentBorder, children, tk, empty }: {
  ...
  return (
    ...
    >
      <div style={{ padding:"14px 16px", borderBottom:`1px solid ${tk.divider}`, background:tk.inset, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <div style={{ width:30, height:30, borderRadius:8, background:accentLight, border:`1px solid ${accentBorder}`, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
        ...
        <div style={{ width:6, height:6, borderRadius:"50%", background:accentColor, opacity:0.5, boxShadow:`0 0 6px ${accentColor}`, flexShrink:0 }}/>
      </div>
      {empty ? (
        <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", minHeight:180, gap:10, opacity:0.25 }}>
...
function InputField({ label, desc, color, colorLight, colorBorder, value, onChange, onEnter, tk }: {
  ...
  return (
    <div style={{ background:tk.card, border:`1.5px solid ${focused ? colorBorder : tk.cardBorder}`, borderRadius:14, overflow:"hidden", boxShadow: focused ? `0 0 0 3px ${colorLight}, ${tk.shadow1}` : tk.shadow1, transition:"all 0.2s ease" }}>
      <div style={{ padding:"11px 14px 9px", borderBottom:`1px solid ${tk.divider}`, background:tk.inset }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:8 }}>
          <span style={{ fontSize:12, fontWeight:800, color, letterSpacing:"-0.01em", fontFamily:"'Clash Display',sans-serif", flexShrink:0 }}>{label}</span>
          <span style={{ fontSize:9, color:tk.ink3, letterSpacing:"0.04em", textAlign:"right", lineHeight:1.3 }}>{desc}</span>
        </div>
      </div>
      <div style={{ display:"flex", alignItems:"center", padding:"12px 14px", gap:6 }}>
        <span style={{ fontSize:14, color:tk.ink4, userSelect:"none", fontWeight:600, flexShrink:0 }}>@</span>
        <input
          value={value}
          ...
          spellCheck={false}
          autoComplete="off"
          style={{ flex:1, border:"none", outline:"none", background:"transparent", fontFamily:"'Plus Jakarta Sans',sans-serif", fontSize:14, fontWeight:600, color:tk.ink, minWidth:0 }}
        />
      </div>
    </div>
  );
...
function RepoCard({ repo, tk }: { repo: RepoItem; tk: Theme }) {
  ...
  return (
    <div
      onMouseEnter={() => setHov(true)}
      onMouseLeave={() => setHov(false)}
      style={{ padding:"16px 18px", borderRight:`1px solid ${tk.divider}`, borderBottom:`1px solid ${tk.divider}`, background: hov ? tk.inset : "transparent", cursor:"default", transition:"background 0.15s" }}
    >
      <div style={{ fontSize:13, fontWeight:700, color:tk.ink, marginBottom:7, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", letterSpacing:"-0.01em" }}>{repo.name}</div>
      <div style={{ display:"flex", gap:12, fontSize:11, color:tk.ink3, fontWeight:600, marginBottom:7 }}>
...
export default function Page() {
  ...
  const tagStyle = (color: string, bg: string, border: string): CSSProperties => ({
    ...
    padding:"4px 11px", borderRadius:6, border:`1px solid ${border}`, color, background:bg,
  ...
  return (
    <>
      ...
      <div style={{ minHeight:"100vh", background:tk.pageBg, color:tk.ink, fontFamily:"'Plus Jakarta Sans',sans-serif", transition:"background 0.4s, color 0.4s", position:"relative" }}>
        <BgMesh tk={tk}/>
        <div style={{ position:"relative", zIndex:1, maxWidth:1100, margin:"0 auto", padding:`0 ${px} 80px` }}>

          {/* ── TOPBAR ── */}
          <header className="fadeUp" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"18px 0", marginBottom: isMobile ? 40 : 64, borderBottom:`1px solid ${tk.divider}` }}>
            {/* Logo */}
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              <div style={{ width:36, height:36, borderRadius:9, backgroundImage:`linear-gradient(135deg, ${tk.blue}, ${tk.purple})`, display:"flex", alignItems:"center", justifyContent:"center", boxShadow:`0 4px 12px ${tk.blueGlow}`, flexShrink:0 }}>
                <span style={{ fontFamily:"'Clash Display',sans-serif", fontSize:16, fontWeight:700, color:"#fff" }}>D</span>
              </div>
              <div>
                <div style={{ fontFamily:"'Clash Display',sans-serif", fontSize:17, fontWeight:700, color:tk.ink, letterSpacing:"-0.03em", lineHeight:1 }}>DevIQ</div>
                {!isMobile && <div style={{ fontSize:9, color:tk.ink3, fontWeight:600, letterSpacing:"0.14em", textTransform:"uppercase", marginTop:1 }}>Intelligence Engine</div>}
              </div>
            </div>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              {!isMobile && (
                <div style={{ display:"flex", alignItems:"center", gap:7, padding:"6px 12px", borderRadius:99, background:tk.pillBg, border:`1px solid ${tk.cardBorder}`, fontSize:11, fontWeight:600, color:tk.ink2 }}>
                  <div style={{ width:6, height:6, borderRadius:"50%", background:tk.green, boxShadow:`0 0 8px ${tk.green}`, animation:"livePulse 2s ease-in-out infinite" }}/>
                  All systems live
                </div>
              )}
              <button
                onClick={() => setDark(d => !d)}
                style={{ display:"flex", alignItems:"center", gap: isMobile ? 0 : 7, padding: isMobile ? "8px" : "7px 14px", borderRadius:99, border:`1.5px solid ${tk.cardBorder}`, background:tk.card, cursor:"pointer", fontSize:12, fontWeight:700, color:tk.ink2, transition:"all 0.2s", boxShadow:tk.shadow1 }}
              >
                <span style={{ fontSize:15 }}>{dark ? "☀️" : "🌙"}</span>
                {!isMobile && (dark ? " Light" : " Dark")}
              </button>
            </div>
          </header>
          ...
          <section style={{ marginBottom: isMobile ? 40 : 72 }}>
            {/* Badge */}
            <div className="fadeUp" style={{ display:"inline-flex", alignItems:"center", gap:7, marginBottom:18, padding:"5px 12px", borderRadius:99, background:tk.blueLight, border:`1px solid ${tk.blueBorder}` }}>
              <div style={{ width:5, height:5, borderRadius:"50%", background:tk.blue, animation:"livePulse 2s ease-in-out infinite", flexShrink:0 }}/>
              <span style={{ fontSize:10, fontWeight:700, letterSpacing:"0.14em", textTransform:"uppercase", color:tk.blue }}>Multi-Platform Developer Analytics</span>
            </div>

            {/* Two-col on desktop, stacked on mobile */}
            <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : "1fr auto", alignItems:"end", gap: isMobile ? 28 : 40 }}>
              <div>
                <h1 className="fadeUp fadeUp1" style={{ fontFamily:"'Clash Display',sans-serif", fontWeight:700, fontSize: isMobile ? "clamp(36px,10vw,56px)" : "clamp(44px,6vw,80px)", lineHeight:1.02, letterSpacing:"-0.04em", color:tk.ink, marginBottom:18 }}>
                ...
                <p className="fadeUp fadeUp2" style={{ fontSize: isMobile ? 14 : 16, color:tk.ink2, lineHeight:1.75, maxWidth:500, fontWeight:400 }}>
                  Connect GitHub, LeetCode &amp; Codeforces. Get a unified intelligence score backed by real data.
                </p>
              </div>

              {/* Stats widget — hidden on mobile */}
              {!isMobile && (
                <div className="fadeUp fadeUp3" style={{ display:"flex", flexDirection:"column", background:tk.card, border:`1px solid ${tk.cardBorder}`, borderRadius:18, overflow:"hidden", boxShadow:tk.shadow2, minWidth:150 }}>
                  {([{ num:"3", label:"Platforms" }, { num:"100", label:"Point Scale" }, { num:"∞", label:"Insights" }] as const).map((s, i, arr) => (
                    <div key={s.label} style={{ padding:"16px 22px", borderBottom: i < arr.length-1 ? `1px solid ${tk.divider}` : "none", textAlign:"center" }}>
                      <div style={{ fontFamily:"'Clash Display',sans-serif", fontSize:30, fontWeight:700, color:tk.ink, letterSpacing:"-0.04em", lineHeight:1 }}>{s.num}</div>
          ...
          {/* ── INPUTS ── */}
          <div className="fadeUp fadeUp2" style={{ marginBottom: isMobile ? 32 : 52 }}>
            <div style={{ fontSize:10, fontWeight:700, letterSpacing:"0.16em", textTransform:"uppercase", color:tk.ink3, marginBottom:10 }}>Connect your profiles</div>
            {/* Stack on mobile, 3-col on desktop */}
            <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : isTablet ? "1fr 1fr" : "repeat(3,1fr)", gap:10, marginBottom:10 }}>
            ...
            <button
              ...
              style={{
                width:"100%", padding: isMobile ? 14 : 17, border:"none", borderRadius:14,
                cursor:(loading||(!gh&&!lc&&!cf)) ? "not-allowed" : "pointer",
                fontFamily:"'Clash Display',sans-serif", fontSize: isMobile ? 14 : 16, fontWeight:700, letterSpacing:"-0.01em",
            ...
            >
              ...
              <span style={{ position:"relative" }}>{loading ? "Analyzing…" : "⚡  Run Intelligence Analysis"}</span>
              {!loading && <span style={{ position:"relative", fontSize:16 }}>→</span>}
            </button>
          </div>

          {/* ── LOADER ── */}
          {loading && (
            <div className="fadeUp" style={{ background:tk.card, border:`1px solid ${tk.cardBorder}`, borderRadius:20, padding: isMobile ? "40px 20px" : "64px 40px", textAlign:"center", marginBottom:20, boxShadow:tk.shadow2 }}>
              <div style={{ width:40, height:40, borderRadius:"50%", border:`3px solid ${tk.cardBorder2}`, borderTopColor:tk.blue, animation:"spin 0.8s linear infinite", margin:"0 auto 24px" }}/>
              <div style={{ fontFamily:"'Clash Display',sans-serif", fontSize: isMobile ? 20 : 26, fontWeight:700, color:tk.ink, letterSpacing:"-0.03em", marginBottom:8 }}>Fetching your data</div>
              <div style={{ fontSize:13, color:tk.ink3, marginBottom:28 }}>Computing your intelligence score…</div>
              <div style={{ display:"flex", justifyContent:"center", gap:8, flexWrap:"wrap" }}>
                {steps.map((s,i) => (
                  <div key={i} style={{ display:"flex", alignItems:"center", gap:6, fontSize:11, fontWeight:700, letterSpacing:"0.06em", textTransform:"uppercase", padding:"6px 14px", borderRadius:99, border:`1.5px solid ${stepBdr(s.status)}`, color:stepColor(s.status), background:stepBg(s.status), transition:"all 0.3s" }}>
          ...
          {errors.length > 0 && !loading && (
            <div style={{ display:"flex", flexDirection:"column", gap:6, marginBottom:16 }}>
              {errors.map((e,i) => (
                <div key={i} style={{ display:"flex", alignItems:"center", gap:10, padding:"11px 16px", borderRadius:10, border:`1px solid ${tk.roseBorder}`, background:tk.roseLight, fontSize:12, fontWeight:600, color:tk.rose }}>
                  ⚠ {e}
                </div>
              ))}
            </div>
          ...
          {data && !loading && (() => {
            ...
            return (
              <>
                {/* ── SCORE BANNER ── */}
                <div className="scalePop" style={{ background:tk.card, border:`1px solid ${tk.cardBorder}`, borderRadius:20, marginBottom:12, boxShadow:tk.shadow3, overflow:"hidden", position:"relative" }}>
                  <div style={{ height:3, backgroundImage:`linear-gradient(90deg, ${tk.blue}, ${tk.purple}, ${tk.teal})` }}/>

                  {/* Mobile: stacked. Desktop: side by side */}
                  <div style={{ padding: isMobile ? "24px 18px" : "36px 40px", display:"grid", gridTemplateColumns: isMobile ? "1fr" : "1fr auto", gap: isMobile ? 28 : 32, alignItems:"center" }}>
                    <div>
                      ...
                      <div style={{ fontFamily:"'Clash Display',sans-serif", fontSize: isMobile ? "clamp(22px,7vw,36px)" : "clamp(28px,4vw,48px)", fontWeight:700, letterSpacing:"-0.04em", color:tk.ink, marginBottom:16, lineHeight:1.05 }}>
                      ...
                      <div style={{ padding:"14px 16px", background:tk.inset, borderRadius:10, border:`1px solid ${tk.divider}`, display:"flex", alignItems:"flex-start", gap:10 }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: isMobile ? 13 : 15, color:tk.ink, fontWeight:500, lineHeight:1.5 }}>{getVerdict(data.combined_score)}</div>
                        </div>
                      </div>
                    </div>

                    {/* Ring — centred on mobile */}
                    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:12, ...(isMobile ? { order:-1 } : {}) }}>
                      <div style={{ fontSize:10, fontWeight:700, letterSpacing:"0.16em", textTransform:"uppercase", color:tk.ink3 }}>Overall Score</div>
                      <ScoreRing score={data.combined_score} tk={tk} size={isMobile ? 120 : 148}/>
                      ...
                      <>
                        ...
                        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8, padding:"12px 14px 14px" }}>
                          {([
                            ...
                            <div key={d.l} style={{ borderRadius:10, padding:"12px 6px", textAlign:"center", border:`1.5px solid ${d.bdr}`, background:d.bg }}>
                              <div style={{ fontFamily:"'Clash Display',sans-serif", fontSize:24, fontWeight:700, lineHeight:1, color:d.c, letterSpacing:"-0.03em" }}>{d.n}</div>
                              <div style={{ fontSize:9, fontWeight:700, letterSpacing:"0.12em", textTransform:"uppercase", color:d.c, marginTop:4, opacity:0.8 }}>{d.l}</div>
                            </div>
                          ))}
                        </div>
                      </>
                ...
                {data.github?.repositories && data.github.repositories.length > 0 && (
                  <div className="scalePop scalePop1" style={{ background:tk.card, border:`1px solid ${tk.cardBorder}`, borderRadius:18, overflow:"hidden", boxShadow:tk.shadow2, marginBottom:12 }}>
                    <div style={{ padding:"14px 20px", borderBottom:`1px solid ${tk.divider}`, background:tk.inset, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                        <div style={{ width:26, height:26, borderRadius:7, background:tk.blueLight, border:`1px solid ${tk.blueBorder}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, flexShrink:0 }}>📁</div>
                        <span style={{ fontFamily:"'Clash Display',sans-serif", fontSize:14, fontWeight:700, color:tk.ink, letterSpacing:"-0.02em" }}>Repositories</span>
                      </div>
                      <span style={{ fontSize:11, color:tk.ink3, fontWeight:600, padding:"3px 10px", borderRadius:99, background:tk.pillBg, border:`1px solid ${tk.cardBorder}`, whiteSpace:"nowrap" }}>{data.github.repositories.length} projects</span>
                    </div>
                    <div style={{ display:"grid", gridTemplateColumns:`repeat(auto-fill, minmax(${isMobile ? "160px" : "200px"}, 1fr))` }}>
                ...
                {/* ── FOOTER ── */}
                <div style={{ padding:"16px 0", borderTop:`1px solid ${tk.divider}`, display:"flex", flexDirection: isMobile ? "column" : "row", justifyContent:"space-between", alignItems: isMobile ? "flex-start" : "center", gap: isMobile ? 6 : 0, fontSize:11, color:tk.ink3, fontWeight:600 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    <div style={{ width:18, height:18, borderRadius:5, backgroundImage:`linear-gradient(135deg,${tk.blue},${tk.purple})`, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                      <span style={{ fontFamily:"'Clash Display',sans-serif", fontSize:9, fontWeight:700, color:"#fff" }}>D</span>
                    </div>
